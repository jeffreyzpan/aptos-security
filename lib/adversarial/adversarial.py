...
from lib.datasets.data_utils import *
import eagerpy as ep
import foolbox as fb

class RegFGSM(fb.attacks.FGSM):
    def get_loss_fn(
        self, model, labels: ep.Tensor):
        def loss_fn(inputs: ep.Tensor) -> ep.Tensor:
            logits = model(inputs)
            return ((logits.flatten()-ep.float32(labels))**2).mean()
...
def gen_attacks(test_loader, classifier, attacks, epsilons, gpu_id_list='0,1,2,3', use_gpu=True):
    ...
    for attack_name, attack in zip(attacks.keys(), attacks.values()):
        ...
        for i, adv_examples in enumerate(adv_list):
            ...
            adv_set = APTOSTensorDataset((adv_examples, test_labels), AdjustContrast(1))
...
def cw_linf(classifier, test_loader, epsilons):
    ...
    for epsilon in epsilons:
        ...
        adv_set = APTOSTensorDataset((torch.from_numpy(adv_examples), torch.from_numpy(test_labels)), AdjustContrast(1))
        adv_loader = torch.utils.data.DataLoader(adv_set, batch_size=128, num_workers=16)
